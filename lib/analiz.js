...
export function macAnaliziOlustur(mac) {
  ...
  function ekBilgileriEkle(metin) {
    if (altUst) {
    ...
    if (h2h && h2h.macSayisi > 0) {
      metin += ` Geçmiş ${h2h.macSayisi} karşılaşmada ${evSahibi} ${h2h.evGalibiyeti} kez, ${misafir} ${h2h.depGalibiyeti} kez kazandı (${h2h.beraberlik} beraberlik).`;
    }
    return metin;
  }
