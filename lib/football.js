...
// Ücretsiz planda erişilebilen / genel olarak takip edilecek ligler.
// League ID'leri API-Football'un kendi dokümantasyonundan alınmıştır.
// Not: Ücretsiz plan günlük 100 istekle sınırlı olduğundan liste kısa tutuldu.
// İhtiyaç oldukça buraya yeni lig ID'leri eklenebilir.
...
// Bugünün tarihini YYYY-MM-DD formatında döner (UTC).
function bugununTarihi() {
  return new Date().toISOString().slice(0, 10);
}

// Belirtilen ligler için bugünkü maçları çeker.
async function bugunkuMaclariGetir() {
  ...
  for (const lig of TAKIP_EDILEN_LIGLER) {
    try {
      ...
      // Bir lig başarısız olursa diğerlerine devam et.
      console.error(`Lig verisi alınamadı (${lig.name}):`, err.message);
    }
  }
...
async function macOranlariniGetir(fixtureId) {
  try {
    ...
    for (const v of market.values) {
      // v.value: "Home" | "Draw" | "Away"
      values[v.value] = parseFloat(v.odd);
    }
    return values; // { Home: 1.85, Draw: 3.40, Away: 4.20 }
  } catch (err) {
    return null;
  }
}

// Basit bir "güven skoru" hesabı: oran ne kadar düşükse
// (yani favori ne kadar netse) güven o kadar yüksek kabul edilir.
// Gerçek bir "value bet" modeli değildir — form/istatistik entegrasyonu
...
function tahminOlustur(mac, oranlar) {
  ...
  // En düşük oranlı seçenek = en "favori" seçenek.
...
// Tahminleri oran aralığına göre 3 kategoriye ayırır ve
// her kategoriden en fazla `limit` adet seçer.
